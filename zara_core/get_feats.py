...
def band_power(psd_freq, psd_val, fmin, fmax):
    ...
    return np.trapezoid(psd_val[idx], psd_freq[idx])
...
def extract_frequency_domain_features(channel_data, name, axis, fs):
    ...
    energy = np.trapezoid(psd ** 2, freqs)
...
def extract_features(sensor_data, fs, channel_names):
    ...
    for p in positions:
        acc_key = f"{p}_acc_mag_rms"
        gyro_key = f"{p}_gyro_mag_rms"
        if acc_key in feats and gyro_key in feats:
            feats[f"{p}_intensity_ratio_acc_gyro"] = feats[acc_key] / (feats[gyro_key] + 1e-9)
